...
def handle_issue_opened(payload, event):
    ...
    if workflow_labels:
        logger.info(
            f"⏭️ Skipping self-created issue #{issue_number} (has workflow label: {workflow_labels})"
        )
        ...
        for _key, _cfg in PROJECT_CONFIG.items():
            if isinstance(_cfg, dict) and repo_name in _project_repos(_key, _cfg, get_repos):
                ...
                if os.path.exists(_task):
                    logger.info(
                        f"⏭️ Skipping issue #{issue_number} — active task file already exists: {_task}"
                    )
...
"""
        ...
        emit_alert(
            f"Issue processing error for #{issue_number}: {str(e)}",
            severity="error",
...
def handle_issue_comment(payload, event):
    ...
    is_completion = any(
        re.search(pattern, comment_body, re.IGNORECASE) for pattern in completion_markers
    )
    ...
    if is_completion and not next_agent:
        ...
        try:
            pid, _ = launch_next_agent(
                issue_number=issue_number, next_agent=next_agent, trigger_source="webhook"
            )
...
def handle_pull_request(payload, event):
    ...
    if action == "opened":
        ...
        if issue_match:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to auto-queue reviewer for issue #{referenced_issue}: {exc}"
                )
    ...
    if action == "closed" and merged:
        review_mode = _effective_review_mode(repo_name)

        # Notify merge only when review mode allows automatic merge handling.
