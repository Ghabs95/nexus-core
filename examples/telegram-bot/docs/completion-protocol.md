...
When an AI agent finishes its work step, it must produce a **completion summary** — a structured report that enables the orchestrator to post rich GitHub comments and auto-chain to the next agent.
...
When using postgres, completions are deduplicated via a `dedup_key` of format `{issue}:{agent_type}:{status}`. If the same agent re-submits for the same issue, the existing row is updated (not duplicated).
